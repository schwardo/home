module("L_BreatheAudio1", package.seeall)
...
local DEN_DID = "urn:breatheaudio-com:serviceId:Receiver1"

local DEVICETYPE_BREATHEAUDIO_AVR_CONTROL = "urn:schemas-breatheaudio-com:device:receiverZone:1"
local DEVICEFILE_BREATHEAUDIO_AVR_CONTROL = "D_BreatheAudioZone1.xml"
...
function set_debug_vars()
    breatheAudioDevice = 'AVR-3808USA'
    childDeviceZone = { Z1 = 'main zone', Z2 = 'zone two', Z3 = 'zone three' }
end

------------------------------------------------------------
function log(text, level)
    luup.log("Breatheaudio AVR plugin: " .. text, (level or 1))
end
...
function sendZoneCommand(device, prefix, command, commandNameString)
    ...
    breatheAudioReceiverSend(prefix .. command)
end

--------------------------------------------------------------------------------
function breatheAudioReceiverSend(command)
    local cmd = command
    ...
    if (luup.io.is_connected == false) then
        log("(breatheAudioReceiverSend) cannot send command " .. command .. " receiver not connected", 1)
        luup.set_failure(true)
        return false
    end
    if (luup.io.write(cmd, breatheAudioDevice) == false) then
        log("(breatheAudioReceiverSend) cannot send command " .. command .. " communications error", 1)
        luup.set_failure(true)
        return false
    end
...
--------------------------------------------------------------------------------
local function initialParameters(breatheAudioDevice)
    luup.variable_set(SWP_SID,SWP_STATUS,"0",breatheAudioDevice)
    -- breatheAudioReceiverSend("PW?")
    ...
    -- breatheAudioReceiverSend("MS?")
    -- breatheAudioReceiverSend("SI?")
    -- breatheAudioReceiverSend("SV?")
    -- breatheAudioReceiverSend("MU?")
    -- breatheAudioReceiverSend("MV?")
end
...
function createChildren()
    ...
    child_devices = luup.chdev.start(breatheAudioDevice)
    luup.chdev.append(breatheAudioDevice, child_devices, "Z4", "Kitchen",
                      DEVICETYPE_BREATHEAUDIO_AVR_CONTROL, DEVICEFILE_BREATHEAUDIO_AVR_CONTROL, "", "", true)
    luup.chdev.sync(breatheAudioDevice, child_devices)

    -- setup lookup table from zone -> child
    findChildren(breatheAudioDevice)
end
...
--    else
        ...
        initialParameters(breatheAudioDevice)

        -- we want to make sure we have time for the child zones to be filled in the zoneNameTable
        luup.call_delay('add_children', 10, breatheAudioDevice)

--        for k, v in pairs(zoneNameTable) do
--            if(k ~= "R1") then
--                local zoneNumber = k:sub(2,2)
--                breatheAudioReceiverSend("Z" .. zoneNumber .. "?")
--            end
--        end

        --Returns model.
--        breatheAudioReceiverSend("SYMO")
        return true
--    end
end
...
local function set_model(modelName)
    ...
    luup.attr_set("name", modelName, breatheAudioDevice)
...
local cmds = {
    ...
    {'^PWON',               function(...) set_power(breatheAudioDevice, 'ON')  end},
    {'^PWSTANDBY',          function(...) set_power(breatheAudioDevice, 'OFF') end},
